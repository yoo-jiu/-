...
from plotly.colors import n_colors # n_colors는 Plotly에서 색상 그라데이션을 만드는 데 사용됩니다.
...
if not df.empty:
    ...
    if not filtered_df.empty:
        ...
        # 2. 색상 설정: 요청에 따라 1위는 빨간색, 2위부터 진한 파랑색 그라데이션
        num_others = len(rank_df) - 1
        
        if num_others > 0:
            # --- 🎨 색상 그라데이션 수정 (2위부터 진한 파랑 -> 연한 파랑) ---
            # 시작 색: rgb(0, 0, 205) - Medium Blue (진한 파랑)
            # 종료 색: rgb(173, 216, 230) - Light Blue (연한 파랑)
            blue_colors = n_colors('rgb(0, 0, 205)', 'rgb(173, 216, 230)', num_others, colortype='rgb')
